...
import { PPh4Ayat2Validation } from '@/validations/pph-4-ayat-2-validation';
import { BadRequestError, NotFoundError } from '@/errors/response-error';
import {
  TPPh4Ayat2CreateFormSchema,
  TPPh4Ayat2ModelPagedResult,
  TPPh4Ayat2SingleModel,
  TPPh4Ayat2UpdateFormSchema,
  toPPh4Ayat2SingleResponse,
  toPagedPPh4Ayat2Response,
} from '@/types/pph-4-ayat-2-type';
// import { AttachmentService } from '@/services/attachment-service';

export class PPh4Ayat2Service {
  static async create(
    data: TPPh4Ayat2CreateFormSchema
  ): Promise<TPPh4Ayat2SingleModel> {
    try {
      const request = PPh4Ayat2Validation.parse(data);
      ...
      const tarifPajak = npwp !== null ? tarifNpwp : tarifNonNpwp;
      const potonganPajak = (tarifPajak / 100) * request.penghasilan_bruto;
      ...
      return toPPh4Ayat2SingleResponse(rows[0]);
    } catch (error) {
  ...
  static async list(
    ...
    idl: string,
    search?: string
  ): Promise<TPPh4Ayat2ModelPagedResult> {
    ...
    const totalCountQuery = `
      ...
      WHERE ($1::text IS NULL OR idl = $1) AND kode_jenis_pajak = 3
      ${searchQuery}
    `;
    ...
    const pageableData = toPagedPPh4Ayat2Response({
      result: rows.map(toPPh4Ayat2SingleResponse),
  ...
  static async listEntry(
    ...
    const totalCountQuery = `
      SELECT COUNT(kode_kegiatan_penghasilan_badan_usaha) as count
      FROM kegiatan_penghasilan_badan_usaha
      WHERE ($1::text IS NULL OR idl = $1) AND kode_jenis_pajak = 3 AND status = 'Entry'
      ${searchQuery}
    `;
  ...
  static async listVerifikasi(
    ...
    const totalCountQuery = `
      SELECT COUNT(kode_kegiatan_penghasilan_badan_usaha) as count
      FROM kegiatan_penghasilan_badan_usaha
      WHERE ($1::text IS NULL OR idl = $1) AND kode_jenis_pajak = 3 AND status = 'Verifikasi'
  ...
  static async listSetor(
    ...
    const totalCountQuery = `
      SELECT COUNT(kode_kegiatan_penghasilan_badan_usaha) as count
      FROM kegiatan_penghasilan_badan_usaha
      WHERE ($1::text IS NULL OR idl = $1) AND kode_jenis_pajak = 3 AND status = 'Setor'
  ...
  static async update(
    kode_kegiatan_penghasilan_badan_usaha: string,
    data: TPPh4Ayat2UpdateFormSchema
  ): Promise<TPPh4Ayat2SingleModel> {
    try {
      const request = PPh4Ayat2Validation.parse(data);
  ...
  static async delete(kode_kegiatan_penghasilan_badan_usaha: string) {
    try {
      ...
      return rows.map(toPPh4Ayat2SingleResponse) as TPPh4Ayat2SingleModel[];
    } catch (error) {
      throw error;
    }
  }
